...
class MultiHopHeteroGCNLayer(nn.Module):
    def __init__(self, in_feats, out_feats, num_hops=4, activation=F.relu):
...
class HeteroGCN(nn.Module):
    def __init__(self, in_feats, hidden_feats, out_feats):
        ...
        self.layer1 = MultiHopHeteroGCNLayer(in_feats, hidden_feats)
        self.layer2 = MultiHopHeteroGCNLayer(hidden_feats, out_feats)
...
# train_hetero_gcn(g, features, edge_pairs_dict, labels_dict, canonical_edge_type_dict, epochs=100, patience=10)


def calculate_auprc(labels, predictions):
    precision, recall, _ = precision_recall_curve(labels, predictions)
    return average_precision_score(labels, predictions), precision, recall

def evaluate_model_with_curves(model, g, features, edge_pairs_dict, labels_dict, canonical_edge_type_dict):
    model.eval()
    total_loss = 0
    criterion = nn.BCELoss()
    all_predictions = []
    ...
    auprc, precision, recall = calculate_auprc(all_labels, all_predictions)
    fpr, tpr, _ = roc_curve(all_labels, all_predictions)
    auroc = roc_auc_score(all_labels, all_predictions)

    return total_loss, auroc, auprc, fpr, tpr, precision, recall
...
def train_and_evaluate_for_seeds(num_seeds, g, features, edge_pairs_dict, labels_dict, canonical_edge_type_dict, epochs=100, patience=10):
    results = {
        "AUROC": [],
        "AUPRC": []
    }
    ...
    for seed in range(num_seeds):
        ...
        for edge_type, edge_pairs in edge_pairs_dict.items():
            ...
            train_pairs, val_pairs, test_pairs, train_labels, val_labels, test_labels = stratified_split(
                edge_pairs, labels, val_size=0.1, test_size=0.1, seed=seed
            )
        ...
        model = HeteroGCN(in_feats, hidden_feats, out_feats).to(device)
        optimizer = torch.optim.Adam(model.parameters(), lr=0.01)
        criterion = nn.BCELoss()
        ...
        for epoch in range(epochs):
            ...
            # Validation
            val_loss, val_auroc, val_auprc, _, _, _, _ = evaluate_model_with_curves(
        ...
        _, test_auroc, test_auprc, fpr, tpr, precision, recall = evaluate_model_with_curves(
            model, g, features, test_pairs_dict, test_labels_dict, canonical_edge_type_dict
        )

        results["AUROC"].append(test_auroc)
        results["AUPRC"].append(test_auprc)

        # Save ROC and PR curves
        np.savetxt(f"roc_curve_seed_{seed}.csv", np.column_stack((fpr, tpr)), delimiter=",")
        np.savetxt(f"pr_curve_seed_{seed}.csv", np.column_stack((precision, recall)), delimiter=",")

    # Compute and save averages and standard deviations
    avg_auroc = np.mean(results["AUROC"])
    std_auroc = np.std(results["AUROC"])
    avg_auprc = np.mean(results["AUPRC"])
    std_auprc = np.std(results["AUPRC"])

    with open("net_Het_results.txt", "w") as f:
        f.write(f"Mean AUROC over {num_seeds} seeds: {avg_auroc:.4f}\n")
        f.write(f"Std Dev AUROC over {num_seeds} seeds: {std_auroc:.4f}\n")
        f.write(f"Mean AUPRC over {num_seeds} seeds: {avg_auprc:.4f}\n")
        f.write(f"Std Dev AUPRC over {num_seeds} seeds: {std_auprc:.4f}\n")

    print(f"\nMean AUROC: {avg_auroc:.4f}, Std Dev: {std_auroc:.4f}")
    print(f"Mean AUPRC: {avg_auprc:.4f}, Std Dev: {std_auprc:.4f}")

# Train the model for 10 seeds
train_and_evaluate_for_seeds(1, g, features, edge_pairs_dict, labels_dict, canonical_edge_type_dict, epochs=1000, patience=10)
